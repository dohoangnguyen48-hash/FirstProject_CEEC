...
router.post('/change-pin', async (req, res) => {
    try {
        ...
        // Kiểm tra PIN trùng
        const pinExists = await RfidCard.findOne({ pin: new_pin });
        if (pinExists) {
            // Nếu phát hiện có người dùng mã này rồi -> Chặn lại ngay!
            return res.status(400).json({ 
                status: "error", 
                allow_access: false, 
...
async function generateUniquePin() {
    ...
    while (!isUnique) {
        ...
        if (!existingPin) {
            isUnique = true; // Số này chưa ai xài, bẻ khóa vòng lặp!
        }
    }
...
router.post('/cards', async (req, res) => {
    try {
        ...
        const autoGeneratedPin = await generateUniquePin();
        
        // 3. Tạo thẻ mới (Mongoose sẽ tự lấy PIN ngẫu nhiên ở trên và status default là 'active')
        const newCard = new RfidCard({ uid: uid, name: name, pin: autoGeneratedPin});
        await newCard.save();
