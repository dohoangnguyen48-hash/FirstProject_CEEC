...
#define MAX_HTTP_RECV_BUFFER 512 //Kích thước buffer gói tin trả về
static char rcv_buffer[MAX_HTTP_RECV_BUFFER];
static int rcv_len = 0;

// Hàm bắt sự kiện. Khi có response từ server trả về, nó sẽ tự động chạy vào hàm này.
esp_err_t _http_event_handler(esp_http_client_event_t *evt) {
    switch(evt->event_id) {
        case HTTP_EVENT_ON_DATA:
            // Chép dữ liệu nhận được vào bộ đệm
            if (rcv_len + evt->data_len < MAX_HTTP_RECV_BUFFER - 1) {
...
void http_send_auth_request(const char* auth_type, const char* credential) {
    ...
    ESP_LOGI(TAG, "Đang gửi data: %s", post_data);

    // Đính kèm cục JSON vào Body và gửi đi
    esp_http_client_set_post_field(client, post_data, strlen(post_data));

    esp_err_t err = esp_http_client_perform(client);
    if (err == ESP_OK) {
        // Nếu gửi thành công, tiến hành kiểm tra gói hàng
        if (rcv_len > 0) {
            ESP_LOGI(TAG, "<<< SERVER TRA VE JSON: %s", rcv_buffer);
            
            // Dùng cJSON để mở gói hàng
            ...
            if (json_response != NULL) {
                ...
                if (oled_msg && oled_msg->valuestring) {
                    ...
                    if (oled_name && oled_name->valuestring) {
                        ...
                        // Quẹt thẻ: Giao toàn quyền cho Bàn phím xử lý đổi PIN
